...
export default function LoginPage() {
  ...
  const handlePasskeyLogin = async () => {
    ...
    setError('')
    try {
      // 1. Get authentication options (no email = discoverable credential)
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 relative">
      ...
      <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow dark:shadow-gray-900/50 w-full max-w-md space-y-6">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <label htmlFor="email" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">メールアドレス</label>
            <input
              id="email"
              type="text"
              value={username}
          ...
          <div>
            <label htmlFor="password" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">パスワード</label>
            <input
              id="password"
              type="password"
              value={password}
