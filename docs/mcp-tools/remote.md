...
- **`max_bytes` パラメータ** (`remote_exec`/`remote_read_file`/`remote_grep`): text 応答の上限バイト数（opt-in）。既定 `None` では無制限。超過時は `head + [... N bytes omitted. <hint> ...] + tail` に切り詰め、続きの取得方法を hint で誘導。
